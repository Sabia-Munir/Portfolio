...
const projects = [
  ...
  },
  {
    ...
    description: 'A stunning restaurant website with interactive 3D scenes, smooth animations, and a modern UI built with React and Three.js. Features animated food scenes and responsive design.',
  ...
  },
  {
    ...
    description: 'A comprehensive hospital management system built with PHP for managing patient records, appointments, and healthcare operations efficiently.',
...
projects.forEach((project, i) => {
  const card = document.createElement('div');
  card.className = 'project-card glass rounded-2xl p-8 relative overflow-hidden animate-on-scroll opacity-0 translate-y-8';
  card.style.transitionDelay = `${i * 150}ms`;
  card.innerHTML = `
    <div class="absolute top-0 right-0 w-40 h-40 rounded-full blur-[80px] opacity-20 hover:opacity-40 transition-opacity duration-500" style="background-color: ${project.color}"></div>
    <div class="relative z-10">
      <div class="flex items-start justify-between mb-6">
        ...
        <a href="${project.github}" target="_blank" rel="noopener noreferrer" class="w-10 h-10 rounded-xl flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-all">
      ...
      <div class="project-features hidden space-y-2 mb-4">
        ${project.features.map(f => `<div class="flex items-center gap-2 text-sm text-gray-400"><svg class="w-3 h-3 text-primary flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg><span>${f}</span></div>`).join('')}
      </div>
      <button class="toggle-features text-xs font-medium mb-4 transition-colors" style="color: ${project.color}">Show features →</button>
      <div class="flex flex-wrap gap-2">
        ${project.tags.map(tag => `<span class="px-3 py-1 rounded-full text-xs font-mono border" style="color: ${project.color}; border-color: ${project.color}30; background: ${project.color}10;">${tag}</span>`).join('')}
      </div>
    </div>
  `;
